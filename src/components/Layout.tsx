...
const LEDGER_CHILDREN: SidebarKey[] = [
  'reconciliation',
  'gl-writeoff-create',
  'gl-writeoff-list',
  'reports',
...
const GL_WRITEOFF_CHILDREN: SidebarKey[] = ['gl-writeoff-create', 'gl-writeoff-list'];
const MANAGE_CHILDREN: SidebarKey[] = [
  'account-group',
  'report-lines',
  'link-account-groups',
  'link-report-lines',
...
export default function Layout({ activeKey, onNavigate, onLogoutClick, children }: LayoutProps) {
  ...
  useEffect(() => {
    ...
    function handleClickOutside(e: MouseEvent) {
      if (companyMenuOpen && companyMenuRef.current && !companyMenuRef.current.contains(e.target as Node)) {
  ...
  function handleSelectCompany(c: string) {
    setCompanyMenuOpen(false);
    if (c === company) return;
    setCompany(c);
  ...
  function handleSelectLanguage(l: 'TH' | 'EN') {
    setLangMenuOpen(false);
  ...
  const LANGUAGE_OPTIONS: { value: 'TH' | 'EN'; label: string }[] = [
    { value: 'TH', label: 'ไทย (TH)' },
    { value: 'EN', label: 'English (EN)' },
  ];

  function handleNavigate(key: SidebarKey) {
    onNavigate(key);
