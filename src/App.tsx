...
function sidebarKeyForView(view: View): SidebarKey {
  switch (view.name) {
    case 'home':
      return 'home';
    case 'settings':
...
function viewForSidebarKey(key: SidebarKey): View {
  ...
  return { name: 'module', module: key as ModuleKey };
}
...
function AppShellRouter() {
  ...
  const [crudData, setCrudData] = useState<Record<ModuleKey, Row[]>>(
    () => Object.fromEntries(MODULE_KEYS.map((key) => [key, [] as Row[]])) as Record<ModuleKey, Row[]>,
  );
  const [reconciliationItems, setReconciliationItems] = useState<ReconciliationItem[]>([]);
  const [glWriteoffEntries, setGlWriteoffEntries] = useState<GlWriteoffEntry[]>([]);
    ...
    async function loadAll() {
      try {
        const [ag, ft, gw, rc, moduleEntries] = await Promise.all([
          ...
          Promise.all(MODULE_KEYS.map(async (key) => [key, await crudModuleApi.list(key)] as const)),
        ]);
        if (cancelled) return;
        setAccountGroups(ag);
        ...
        setCrudData(Object.fromEntries(moduleEntries) as Record<ModuleKey, Row[]>);
      } catch (err) {
        if (cancelled) return;
  ...
  function persist<T>(run: () => Promise<T>) {
    run().catch((err) => {
