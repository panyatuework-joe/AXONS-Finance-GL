...
function seedMonthlyAmounts(seedIndex: number): number[] {
  return Array.from({ length: 12 }, (_, m) => (seedIndex + 1) * 10000 + m * 1500);
}

export function seedFinancialTargets(): FinancialTarget[] {
...
export function seedAccountGroups(): AccountGroup[] {
...
export const MODULE_CONFIGS: Record<ModuleKey, CrudModuleConfig> = {
  reports: {
    key: 'reports',
    title: 'รายงาน',
...
function row(id: string, fields: Record<string, string>): Row {
  return { id, ...fields };
}

export function seedModuleData(): Record<ModuleKey, Row[]> {
  return {
    reports: [
...
const RECONCILIATION_SEED: Omit<ReconciliationItem, 'status' | 'lastChecked'>[] = [
...
export function seedReconciliationItems(): ReconciliationItem[] {
