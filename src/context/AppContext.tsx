...
import { translate, translateVars, type Language } from '../i18n';
...
type DownloadStage = 'preparing' | 'downloading' | 'complete' | 'failed';
...
type ToastItem = MessageToast | DownloadToast;
...
interface AppContextValue {
  ...
  pushToast: (message: string, type?: 'success' | 'error', detail?: string) => void;
  startDownloadToast: (filename: string, run: () => void) => void;
  closeToast: (id: string) => void;
  clearToasts: () => void;
  t: (text: string) => string;
  tv: (template: string, vars: Record<string, string | number>) => string;
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const tv = useCallback(
    (template: string, vars: Record<string, string | number>) => translateVars(template, vars, language),
    [language],
  );

  const pushToast = useCallback(
    (message: string, type: 'success' | 'error' = 'success', detail?: string) => {
