...
interface Props {
  financialTargetsCount: number;
  reconciliationCount: number;
  accountGroupsCount: number;
  crudData: Record<ModuleKey, Row[]>;
  onNavigateModule: (key: ModuleKey | 'financial-target' | 'reconciliation' | 'account-group') => void;
}
...
export default function HomePage({
  ...
  const { t } = useApp();
  const remainingModuleKeys = (Object.keys(MODULE_CONFIGS) as ModuleKey[]).filter((key) => key !== 'reports');
