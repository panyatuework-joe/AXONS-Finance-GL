...
let lineIdCounter = 0;
function newLine(accountCode: string): GlWriteoffLine {
  lineIdCounter += 1;
  return { id: `glw-line-${Date.now()}-${lineIdCounter}`, dept: '', accountCode, cvCode: '', amount: 0 };
}

export default function GlWriteoffFormPage({ existing, onCancel, onSave }: Props) {
  const { t } = useApp();
  ...
  const [debitLines, setDebitLines] = useState<GlWriteoffLine[]>([newLine(GL_ACCOUNT_CODE_OPTIONS[0])]);
  const [creditLines, setCreditLines] = useState<GlWriteoffLine[]>([newLine(GL_ACCOUNT_CODE_OPTIONS[1])]);
  ...
  const linesValid = (lines: GlWriteoffLine[]) =>
  ...
  function buildEntry(): GlWriteoffEntry {
  ...
  function renderLineTable(kind: 'debit' | 'credit') {
