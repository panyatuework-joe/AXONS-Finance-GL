...
interface Props {
  ...
  onChange: (rows: FinancialTarget[]) => void;
  onAdd: () => void;
  onView: (id: string) => void;
  onEdit: (id: string) => void;
}

export default function FinancialTargetPage({ data, onChange, onAdd, onView, onEdit }: Props) {
  const { pushToast, t, tv } = useApp();
  const [year, setYear] = useState('all');
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  const [deleteTarget, setDeleteTarget] = useState<FinancialTarget | null>(null);
  const [deleteSuccessOpen, setDeleteSuccessOpen] = useState(false);

  // Applied filter values (actually used to filter the table)
  const [appliedDepts, setAppliedDepts] = useState<string[]>([]);
  const [appliedSubDepts, setAppliedSubDepts] = useState<string[]>([]);
  const [appliedAccountCodes, setAppliedAccountCodes] = useState<string[]>([]);

  // Draft values edited inside the filter modal (committed on "ค้นหา")
  const [filterModalOpen, setFilterModalOpen] = useState(false);
  const [draftDepts, setDraftDepts] = useState<string[]>([]);
  const [draftSubDepts, setDraftSubDepts] = useState<string[]>([]);
  ...
  function removeAppliedChip(kind: 'dept' | 'subDept' | 'accountCode', value: string) {
