...
interface Props {
  mode: 'add' | 'edit';
  initial?: FinancialTarget;
  existing: FinancialTarget[];
  onCancel: () => void;
  onSave: (ft: FinancialTarget) => void;
}

export default function FinancialTargetFormPage({ mode, initial, existing, onCancel, onSave }: Props) {
  const { t } = useApp();
  ...
  const formatTotal = (n: number) => n.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  ...
  function validateFields(): boolean {
    ...
    const amtErrors: Record<number, string> = {};
    amounts.forEach((a, i) => {
  ...
  function buildFinancialTarget(): FinancialTarget {
    return {
