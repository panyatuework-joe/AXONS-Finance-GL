...
type SortableKey = Exclude<keyof DetailRow, 'id'>;

function generateDetailRows(item: ReconciliationItem, seed: number): DetailRow[] {
...
interface Props {
  ...
  onChange: (updated: ReconciliationItem) => void;
  onBack: () => void;
}

export default function ReconciliationDetailPage({ item, onChange, onBack }: Props) {
  ...
  function exportCsv(rows: DetailRow[], filename: string) {
