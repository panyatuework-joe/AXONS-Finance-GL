...
type SortKey = 'code' | 'nameTH' | 'nameEN' | 'allowEdit' | 'carryForward' | 'createdBy' | 'createdAt' | 'updatedAt';
...
function isWithinDateRange(value: string, start: string, end: string): boolean {
...
interface Props {
  data: AccountGroup[];
  onChange: (rows: AccountGroup[]) => void;
}
...
const EMPTY_FORM: FormState = { code: '', nameTH: '', nameEN: '', allowEdit: false, carryForward: false };

export default function AccountGroupPage({ data, onChange }: Props) {
  const { t, tv } = useApp();
  ...
  const [appliedNameTH, setAppliedNameTH] = useState<string[]>([]);
  const [appliedNameEN, setAppliedNameEN] = useState<string[]>([]);
  ...
  const [draftNameTH, setDraftNameTH] = useState<string[]>([]);
  const [draftNameEN, setDraftNameEN] = useState<string[]>([]);
  const [draftCreatedBy, setDraftCreatedBy] = useState<string[]>([]);
  ...
  const [editingRow, setEditingRow] = useState<AccountGroup | null>(null);
  const [form, setForm] = useState<FormState>(EMPTY_FORM);
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [dirty, setDirty] = useState(false);

  const [viewTarget, setViewTarget] = useState<AccountGroup | null>(null);
